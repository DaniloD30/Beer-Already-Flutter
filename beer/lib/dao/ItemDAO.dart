...
class ItemDAO {
  ...
  static Future<List<Item>> retrieveAll() async {
    ...
    if (response.statusCode == 200) {
      ...
      for (int x = 0; x < estabelecimentoJson.length; x++)
        estabelecimentoList.add(Item.fromJson(estabelecimentoJson[x] as Map<String, dynamic>));
  ...
  static Future<Item> update(Item estabelecimento) async {
    final response = await http
        .patch('https://danilod.pythonanywhere.com/item/' + estabelecimento.id.toString()
        + "/",
        headers: {"Content-Type": "application/json"},
  ...
  static Future<bool> remove(int id) async {
    final response = await http
        .delete('https://danilod.pythonanywhere.com/item/' + id.toString() + "/");
